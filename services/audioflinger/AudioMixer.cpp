...
#define MixerDumpPcm(name, propty, pid, tid, value, buffer, size, format, sampleRate, channelCount ) \
{\
  char fileName[256]; \
  sprintf(fileName,"%s_pid%d_tid%d_%p.pcm", name, pid, tid, (void*)value); \
        ...
        mSampleRate(sampleRate)
{
    ...
    mState.resampleTemp = new int32_t[MAX_NUM_CHANNELS * mState.frameCount];
    ALOGD("resampleTemp %p, downMixBuffer %p, size %zu(int32 )", mState.resampleTemp,
        mState.downMixBuffer, mState.frameCount * MAX_NUM_CHANNELS);
#endif
#ifdef MTK_AUDIOMIXER_ENABLE_DRC
    ...
    if (!mState.nonResampleTemp) {
        ...
        ALOGD("%s, new nonResampleTemp %p", __FUNCTION__, mState.nonResampleTemp);
    }
#endif
    // mState.reserved
    ...
    for (unsigned i=0 ; i < MAX_NUM_TRACKS ; i++) {
        ...
        t->mCrossMountLocalPlayback = false;
        t->mCrossMountSpeakerRole = CROSS_MOUNT_SPEAKER_ROLE_NONE;
#endif
        t++;
    }
...
status_t AudioMixer::track_t::prepareTrackForSurroundMix()
{
    ...
    if (AudioMixer::BesSurroundTrackCount >= 3) {
        ALOGV("prepareTrackForSurroundMix , no surround track available, BesSurroundTrackCount %d, trackName %p",
            AudioMixer::BesSurroundTrackCount, this);
        return NO_INIT;
    }
...
void AudioMixer::track_t::unprepareTrackForSurroundMix()
{
    if (mSurroundMixer != NULL) {
        ...
        mSurroundMixer = NULL;
        AudioMixer::BesSurroundTrackCount--;
        ALOGV("unprepareTrackForSurroundMix BesSurroundTrackCount-- %d, trackName %p",
            AudioMixer::BesSurroundTrackCount, this);
    } else {
...
int timetotal;
#endif
AudioMixer::MTKTimeStretchBufferProvider::MTKTimeStretchBufferProvider(int framecount, track_t* pTrack)
    :   PassthruBufferProvider(), mTrackBufferProvider(NULL), mTimeStretchHandle(NULL), mOutBuffer(NULL),
        mOutRemain(0)
{
    if (framecount > 0) {
    ...
    ALOGV("new Timestretch, internal input buffer framecount %zu ", mOutframecount);
    ...
    mInputBufferProvider = NULL;
    memset(&mTimeStretchConfig, 0, sizeof(mTimeStretchConfig));
}
...
void AudioMixer::MTKTimeStretchBufferProvider::releaseBuffer(AudioBufferProvider::Buffer *pBuffer)
{
    ...
    if (mOutRemain  == 0 && pBuffer->frameCount != 0) {
        ...
        ALOGVV("release pBuffer-> raw %p, pBuffer->frameCount %d", pBuffer->raw, pBuffer->frameCount);
        ...
        ALOGVV("release mBuffer-> raw %p, mBuffer->frameCount %d", mBuffer.raw, mBuffer.frameCount);
...
status_t AudioMixer::MTKTimeStretchBufferProvider::getNextBuffer(AudioBufferProvider::Buffer *pBuffer)
{
    ...
    status_t res;
    ALOGD("MTKTimeStretchBufferProvider::getNextBuffer() this %p", this);

    if (mTimeStretchHandle == NULL) {
        ALOGE("MTKTimeStretchBufferProvider::getNextBuffer() error: NULL handler");
        return NO_INIT;
    }
    ...
    if (mOutRemain != 0) {
        ...
        ALOGV("MTKTimeStretchBufferProvider::getNextBuffer() directly return %zu", pBuffer->frameCount);
        pBuffer->raw = mOutBuffer;
        return OK;
    }
    ...
    if (mTimeStretchHandle->mBTS_RTParam.TS_Ratio != 100) {
    ...
    pBuffer->frameCount = mTimeStretchHandle->InternalBufferSpace() > pBuffer->frameCount
        ? pBuffer->frameCount : mTimeStretchHandle->InternalBufferSpace();
    ...
    /////////Get data////////////////////////////////////////////////
    ALOGV("Timestertch getNextBuffer real required %zu", pBuffer->frameCount);
    mBuffer.frameCount = pBuffer->frameCount;
    mBuffer.raw = pBuffer->raw;
    res = mTrackBufferProvider->getNextBuffer(&mBuffer);
    ...
    const int SIZE = 256;
    char fileName[SIZE];
    sprintf(fileName, "%s_%p.pid%d.tid%d.pcm", gaf_timestretch_in_pcm, this, getpid(), gettid());
    ...
    ALOGV("Timestertch getNextBuffer real get %zu", mBuffer.frameCount);
    ...
    if (second_request && dataGet != 0) {
        ...
        ALOGVV("mBuffer.raw %p pBuffer-> raw %p", mBuffer.raw, pBuffer->raw);
        memcpy(mInBuffer, mBuffer.raw, mBuffer.frameCount << ChannelCount);
        mTrackBufferProvider->releaseBuffer(&mBuffer);
        mBuffer.frameCount = second_request;
        ALOGVV("mBuffer.raw %p, mBuffer.frameCount %d", mBuffer.raw, mBuffer.frameCount);
        mTrackBufferProvider->getNextBuffer(&mBuffer);
        if (mBuffer.frameCount) {
            memcpy(mInBuffer + dataGet * ChannelCount, mBuffer.raw, mBuffer.frameCount << ChannelCount);
        }
        ALOGV("second_request real get %zu", mBuffer.frameCount);
        dataGet += mBuffer.frameCount;
#ifdef MTK_AUDIO
#ifdef DEBUG_AUDIO_PCM
        if (mBuffer.frameCount) {
...
status_t AudioMixer::track_t::initTrackMTKTimeStretch(int ratio)
{
    ...
    if (ratio > 55) {
        ...
        if (tsbufferProvider == NULL) {
            ...
            ALOGVV("initTrackMTKTimeStretch( ratio= %d) calls TimeStretchConfig()", ratio);
...
{
    ALOGV("AudioMTKMixer::prepareTrackForMTKTimeStretch(%p) with ratio %d, framecount %d", this, ratio, framecount);

    // discard the previous downmixer if there was one
    unprepareTrackForMTKTimeStretch();

    if (!isTimeStretchCapable) {
        ALOGE("prepareTrackForMTKTimeStretch(%p) fails: mixer doesn't support TimeStretch ", this);
        return NO_INIT;
    }
    if (bufferProvider == NULL) {
        ALOGE("prepareTrackForMTKTimeStretch(%p) fails: pTrack->bufferProvider is null, pTrack %p", this, this);
        return NO_INIT;
    }
    ...
    if (mReformatBufferProvider != NULL) {
        ...
        ALOGV("reset track buffer provider if reformat is used  pTrack->bufferProvider %p", bufferProvider);
    }
    ...
    if (mReformatBufferProvider) {
        ...
        ALOGV("set reformat buffer provider after time stretch  pTrack->bufferProvider %p", bufferProvider);
    }

    ALOGV("prepareTrackForMTKTimeStretch, pTrack->bufferProvider : %p  pTrack->mTrackBufferProvider %p ",
        bufferProvider, pDbp->mTrackBufferProvider);
    return NO_ERROR;
        ...
        case STEREO2MONO:
            ...
            if (track.mSteroToMono != (BLOUD_S2M_MODE_ENUM)valueInt) {
                if (track.mpDRCObj != NULL) {
                    track.mpDRCObj->setParameter(BLOUD_PAR_SET_STEREO_TO_MONO_MODE, (void *)(uintptr_t)valueInt);
                }
            ...
            break;
#ifdef MTK_CROSSMOUNT_MULTI_CH_SUPPORT
        case CROSSMOUNT_LOCALPLAY:
            track.mCrossMountLocalPlayback = valueInt;
            break;
        case SPEAKER_ROLE:
            track.mCrossMountSpeakerRole = (CROSS_MOUNT_SPEAKER_ROLE)valueInt;
            break;
#endif
        default:
...
// Fix dead code issue for Coverity
#ifndef MTK_AUDIO
        else {
            if (state->outputTemp) {
...
#ifdef MTK_BESSURROUND_ENABLE
        if (t ->mSurroundMixer) {
            memcpy_by_audio_format(temp, AUDIO_FORMAT_PCM_32_BIT, temp, AUDIO_FORMAT_PCM_16_BIT,
                outFrameCount * t->channelCount);
            t->mSurroundMixer->process(temp, (t->mDownMixBuffer), outFrameCount);
            memcpy_by_audio_format(temp, AUDIO_FORMAT_PCM_16_BIT, (t->mDownMixBuffer), AUDIO_FORMAT_PCM_32_BIT,
                outFrameCount * 2);
        }
#endif
...
#ifdef MTK_BESSURROUND_ENABLE
            if (t ->mSurroundMixer) {
                memcpy_by_audio_format(out, AUDIO_FORMAT_PCM_32_BIT, out, AUDIO_FORMAT_PCM_16_BIT,
                    outFrameCount * t->channelCount);
                t->mSurroundMixer->process(out, (t->mDownMixBuffer), outFrameCount);
                memcpy_by_audio_format(out, AUDIO_FORMAT_PCM_16_BIT, (t->mDownMixBuffer), AUDIO_FORMAT_PCM_32_BIT,
                    outFrameCount * 2);
            }
#endif
        }
...
#ifdef MTK_BESSURROUND_ENABLE
    if (t ->mSurroundMixer) {
        memcpy_by_audio_format(temp, AUDIO_FORMAT_PCM_32_BIT, in, AUDIO_FORMAT_PCM_16_BIT,
            frameCount * t->channelCount);
        t->mSurroundMixer->process(temp, (t->mDownMixBuffer), frameCount);
        memcpy_by_audio_format((int16_t*)in, AUDIO_FORMAT_PCM_16_BIT, (t->mDownMixBuffer), AUDIO_FORMAT_PCM_32_BIT,
            frameCount * 2);
    }
#endif
...
void AudioMixer::process__genericNoResampling(state_t* state)
{
    ...
    while (e0) {
        ...
        do {
            ...
            MixerDumpPcm(gaf_mixer_end_pcm, gaf_mixer_end_propty, getpid(), gettid(),
                static_cast<int>(reinterpret_cast<uintptr_t>(out)), out,
...
void AudioMixer::process__genericResampling(state_t* state)
{
    ...
    while (e0) {
        ...
        int32_t *out = t1.mainBuffer;
#ifdef MTK_BESSURROUND_ENABLE
        memset(outTemp, 0, sizeof(*outTemp) * (t1.channelCount >= t1.mMixerChannelCount
            ? t1.channelCount : t1.mMixerChannelCount) * state->frameCount);
#else
...
#ifdef MTK_BESSURROUND_ENABLE
        if (t.mSurroundMixer) {
            memcpy_by_audio_format(state->resampleTemp, AUDIO_FORMAT_PCM_32_BIT, in, t.mMixerFormat,
                outFrames * t.channelCount);
            t.mSurroundMixer->process(state->resampleTemp, (t.mDownMixBuffer), outFrames);
            memcpy_by_audio_format((void*)in, t.mMixerFormat, (t.mDownMixBuffer), AUDIO_FORMAT_PCM_32_BIT,
                outFrames * 2);
        }
#endif
...
#ifdef MTK_AUDIO
            if (t->volumeInc[0] != 0) {
                ALOGD("%s, stream %d, vol %d, volinc %d = %zu", __FUNCTION__,
...
#endif
        MixerDumpPcm(gaf_mixer_end_pcm, gaf_mixer_end_propty, getpid(), gettid(), (int)out, out,
...
void AudioMixer::track__Resample(track_t* t, TO* out, size_t outFrameCount, TO* temp, TA* aux)
{
...
#endif
    {
        ...
        if (t ->mSurroundMixer) {
            ...
            memcpy_by_audio_format(out, is_same<TI,
                int16_t>::value ? AUDIO_FORMAT_PCM_16_BIT : AUDIO_FORMAT_PCM_FLOAT,
    ...
    uint32_t MixerChannelCount, uint32_t sampleRate, int32_t* outTemp, int trackCount)
{
    ...
    MixerDumpPcm(gaf_mixer_limin_pcm_before, gaf_mixer_limin_propty, getpid(), gettid(), 0, outTemp, inSize,
    ...
    MixerDumpPcm(gaf_mixer_limin_pcm_after, gaf_mixer_limin_propty, getpid(), gettid(), 0, outTemp, outSize,
        DumpFormat, sampleRate, MixerChannelCount );
}
#else
void AudioMixer::applyLimiter(state_t* state __unused, size_t framecount __unused, audio_format_t MixerInFormat __unused,
    uint32_t MixerChannelCount __unused, uint32_t sampleRate __unused, int32_t* outTemp __unused, int trackCount __unused)
{
}
#endif
...
inline void CopyChannel(void *ptr, size_t frameCount, AudioMixer::CROSS_MOUNT_SPEAKER_ROLE SpeakerRole)
{
    TI *buffer = (TI *)ptr;
    if (SpeakerRole == AudioMixer::CROSS_MOUNT_SPEAKER_ROLE_L_ONLY) {
        while (frameCount) {
            buffer[frameCount - 1] = buffer[frameCount - 2]; // Right Channel = Left Channel
            frameCount -= 2;
        }
    } else if (SpeakerRole == AudioMixer::CROSS_MOUNT_SPEAKER_ROLE_R_ONLY) {
        while (frameCount) {
            buffer[frameCount - 2] = buffer[frameCount - 1]; // Left Channel = Right Channel
            frameCount -= 2;
        }
    }
}
...
{
    DRC_ALOGD("+%s, mStreamType %d", __FUNCTION__, mStreamType);

    int32_t process_channel = (MIXTYPE == MIXTYPE_MONOEXPAND ? channelCount : mMixerChannelCount);

    if (mStreamType == AUDIO_STREAM_PATCH) {
#ifdef MTK_CROSSMOUNT_MULTI_CH_SUPPORT
        if (mCrossMountLocalPlayback && process_channel == 2) {
            if (format == AUDIO_FORMAT_PCM_16_BIT) {
                CopyChannel<int16_t>(buffer, frameCount * process_channel, mCrossMountSpeakerRole);
            } else {
                CopyChannel<int32_t>(buffer, frameCount * process_channel, mCrossMountSpeakerRole);
            }
        }
#endif
        return true;
...
#ifdef DEBUG_AUDIO_PCM_FOR_TEST
    {
        ...
        String8 fileName = String8::format("%s.beforePostProcess.pid%d.tid%d", gaf_mixertest_in_pcm, getpid(), gettid());
    ...
    if ((FCC_2 == mMixerChannelCount) &&
        (SurroundMix_enable || DRC_enable)) {
        DRC_ALOGD("%s, format %d, channel %d %d, frameCount %d, MIXTYPE %d", __FUNCTION__, format,
            mMixerChannelCount, channelCount, frameCount, MIXTYPE);
        ...
        if (SurroundMix_enable) {
            ...
            doBliSrc(mBliSrcDown, processBuffer, frameCount * process_channel * audio_bytes_per_sample(process_format),
                &pBufferAfterBliSrc, &bytesAfterBliSrc);
            mSurroundMixer->process((int32_t*)pBufferAfterBliSrc, (int32_t*)buffer, frameCountAdaptor);
            doBliSrc(mBliSrcUp, buffer, frameCountAdaptor * mMixerChannelCount * audio_bytes_per_sample(process_format),
                &pBufferAfterBliSrc, &bytesAfterBliSrc);
            ALOGV("%s, frameCount %zu, frameCountAdaptor %zu, process_channel %d, bytesAfterBliSrc %d",
                __FUNCTION__, frameCount, frameCountAdaptor, process_channel, bytesAfterBliSrc);
...
#ifdef DEBUG_AUDIO_PCM_FOR_TEST
            {
                String8 fileName = String8::format("%s.surroundProcess.pid%d.tid%d", gaf_mixertest_in_pcm, getpid(), gettid());
                AudioDump::threadDump(fileName.string(), buffer,
...
#if 0 // Only work on duplicated thread with cross mount. So here is no need.
#ifdef MTK_CROSSMOUNT_MULTI_CH_SUPPORT
        if (mCrossMountLocalPlayback && process_channel == 2) {
            if (process_format == AUDIO_FORMAT_PCM_16_BIT) {
                CopyChannel<int16_t>(buffer, frameCount * process_channel, mCrossMountSpeakerRole );
            } else {
                CopyChannel<int32_t>(buffer, frameCount * process_channel, mCrossMountSpeakerRole );
            }
...
#ifdef MTK_HIFI_AUDIO
    if (mBliSrcAdaptorState && (OUTPUT_RATE_192 == mDevSampleRate || OUTPUT_RATE_96 == mDevSampleRate ||
        OUTPUT_RATE_176_4 == mDevSampleRate || OUTPUT_RATE_88_2 == mDevSampleRate)) {
        void *pBufferAfterBliSrc = NULL;
        uint32_t bytesAfterBliSrc = 0;
        size_t frameCountAdaptor = frameCount;
        ...
        doBliSrc(mBliSrcAdaptor, buffer, frameCount * process_channel * audio_bytes_per_sample(AUDIO_FORMAT_PCM_32_BIT),
            &pBufferAfterBliSrc, &bytesAfterBliSrc);
        frameCountAdaptor = frameCountAdaptor >> mBliSrcAdaptorShift;
        memcpy_by_audio_format(buffer, format, pBufferAfterBliSrc, AUDIO_FORMAT_PCM_32_BIT,
            frameCountAdaptor * process_channel);
    } else {
        mBliSrcAdaptorShift = 0;
    }
#endif
...
status_t AudioMixer::track_t::initBliSrc()
{
    ...
    if (OUTPUT_RATE_192 == mDevSampleRate || OUTPUT_RATE_96 == mDevSampleRate ||
        OUTPUT_RATE_176_4 == mDevSampleRate || OUTPUT_RATE_88_2 == mDevSampleRate) {
        uint32_t destSampleRate = (OUTPUT_RATE_192 == mDevSampleRate || OUTPUT_RATE_96 == mDevSampleRate)
...
status_t AudioMixer::track_t::deinitBliSrc()
{
    ...
    if (mBliSrcOutputBuffer != NULL) {
        ...
        mBliSrcOutputBufferSize = 0;
    }

    return NO_ERROR;
}

status_t AudioMixer::track_t::doBliSrc(MtkAudioSrc* mBliSrc, void *pInBuffer, uint32_t inBytes,
    void **ppOutBuffer, uint32_t *pOutBytes)
{
    if (mBliSrc == NULL) { // No need SRC
        ...
        *pOutBytes = inBytes;
        ...
        uint32_t num_converted_data = mBliSrcOutputBufferSize; // max convert num_free_space
        ...
        mBliSrc->multiChannelProcess((int16_t *)p_read, &num_raw_data_left,
...
void AudioMixer::track_t::updateDRCParam(int devSampleRate)
{
    if (mpDRCObj) {
        ...
        mpDRCObj->setParameter(BLOUD_PAR_SET_SAMPLE_RATE, (void *)(uintptr_t)devSampleRate);
...
{
    DRC_ALOGD("setDRCHandler, mUIDRCEnable %d, mpDRCObj %p, mStreamType %d, device %d, mDRCState %d, mSteroToMono %d, this %p",
        mUIDRCEnable, mpDRCObj, mStreamType, device, mDRCState, mSteroToMono, this);
    ...
    if ((true == mUIDRCEnable) && (device & AUDIO_DEVICE_OUT_SPEAKER) &&
    ...
    audio_format_t process_format, int process_channel)
{
    ...
    MixerDumpPcm(gaf_mixer_drc_pcm_before, gaf_mixer_drc_propty, getpid(), gettid(), this, ioBuffer, SampleSize,
        process_format, mDevSampleRate, process_channel );
    ...
    MixerDumpPcm(gaf_mixer_drc_pcm_after,  gaf_mixer_drc_propty, getpid(), gettid(), this, tempBuffer, SampleSize,
        process_format, mDevSampleRate, process_channel );
