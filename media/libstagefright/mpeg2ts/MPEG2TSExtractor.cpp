...
status_t MPEG2TSSource::start(MetaData *params) {
    ...
    if (params && params->findInt32(kKeyDecoderError, &val)) {
        if (val == 1) {
            mExtractor->mParser->setDecoderError(true);
            ALOGE("Audio decoder err!");
        } else {
            mExtractor->mParser->setDecoderError(false);
            ALOGE("Video decoder err!");
        }
        return OK;
    }
#endif
...
#else
    if (mImpl->getQueuedBufferSize() > 200 * 1024 * 1024) {
        ALOGD("AnotherPacketSource queued buffers > 200m, return ERROR_UNSUPPORTED to avoid low memory NE");
...
MPEG2TSExtractor::MPEG2TSExtractor(const sp<DataSource> &source)
    ...
    if(mDataSource->flags() & DataSource::kIsCachingDataSource){
        mParser = new ATSParser(0x80000000); // http streaming is needed
    }else{
...
void MPEG2TSExtractor::init() {
    ...
    off64_t size;
#ifdef MTK_AOSP_ENHANCEMENT // http streaming complete duration is needed, use google default duration
    if ((mDataSource->flags() & DataSource::kIsCachingDataSource)&&
...
#ifdef MTK_AOSP_ENHANCEMENT // http streaming complete duration is needed
...
status_t MPEG2TSExtractor::feedMore(bool isInit) {
    ...
    if (kFillPacketSize == kM2TSPacketSize) {
        ...
        if (event.hasReturnedData()) { // http streaming seek is needed
            if (isInit) {
...
#endif //end of #ifdef SUPPORT_M2TS

#else //end of #ifdef MTK_AOSP_ENHANCEMENT
...
// http streaming seek is needed, http streaming use google default seek
...
bool SniffMPEG2TS(const sp<DataSource> &source, String8 * mimeType,
    ...
    if (retb) {
        ...
        //kFillPacketSize = kTSPacketSize;// not use
    } else {
        retb = true;
        for (int i = 0; i < 5; ++i) {
            char header[5];
        ...
        if (retb) {
            ...
            //kFillPacketSize = kM2TSPacketSize;// not use
        }
    }
    ...
    for (off64_t i = 1; i <= counts; i++) {
        ...
        if (findPAT()) {        //find last PAT
            ...
            while (feedMore() == OK) {  //[qian]the end of file? parse all the TS packet of this file?
                ...
                if (((mOffset - mOffsetPAT) > (10000 * (off64_t) kFillPacketSize)) && (mParser->getMaxPTS() == 0)) {
...
bool MPEG2TSExtractor::IsLocalSource() { // http streaming seek is needed
    if(!(mDataSource->flags() & DataSource::kIsCachingDataSource)){
       return true;
    }
    return false;
