...
namespace android {
#ifdef MTK_AOSP_ENHANCEMENT
bool ElementaryStreamQueueLogOpen = false;  // for log reduction
#endif
...
ElementaryStreamQueue::ElementaryStreamQueue(Mode mode, uint32_t flags)
    : mMode(mode),
      ...
      if (atoi(value)) {
          ElementaryStreamQueueLogOpen = true;
      }
#endif
}
    ...
    while ((err = getNextNALUnit(&data, &size, &nalStart, &nalSize)) == OK) {
        ...
        if (flush) {
            ...
            for (int i = 0; i < preVCLIndex; ++i) {
                ...
                if(!mSeeking||
                    ...
                    // NAL_UNIT_FILLER_DATA = 38, filler data, decoder no use, drop it.
                    if (sliceType == 38) {
                        ALOGW("drop naltype %u, decoder no use", sliceType);
                        continue;
                    }
        ...
        if((mSeeking)){
           if((((sliceType >= 16) && (sliceType <= 21))||((sliceType >= 32) && (sliceType <= 34)))){
            ...
            nals.push_back(pos);
            if (sliceType != 38)  // nal 38 will be dropped
                totalSize += nalSize;
        }
            ...
            if (flush) {
                ...
                for (size_t i = 0; i < nals.size(); ++i) {
                    ...
                    if (nalType == 6) {
                        ...
                        if (ElementaryStreamQueueLogOpen) {
