    ...
    struct ListTableEntries {
        ...
        void popTopTableEntry(){
            ...
            if((0 == (mTotalNumTableEntries % mElementCapacity))) {
                ...
                if (mTempFile != NULL && mTempFileSize>0) {
                    CHECK_GE(mTempFileSize, sizeof(TYPE) * ENTRY_SIZE * mElementCapacity);
                    fseeko(mTempFile, mTempFileSize - sizeof(TYPE) * ENTRY_SIZE * mElementCapacity, SEEK_SET);
                    ALOGD("[%s] popTopTableEntry fread+, mTempFileSize=%zu", mTempFileName.string(), mTempFileSize);
                    int ret = fread(mCurrTableEntriesElement, sizeof(TYPE) * ENTRY_SIZE, mElementCapacity, mTempFile);
                    ALOGD("[%s] popTopTableEntry fread-, read size=%zu, ret=%d",
                        mTempFileName.string(), sizeof(TYPE) * ENTRY_SIZE * mElementCapacity, ret);
                    mTempFileSize -= sizeof(TYPE) * ENTRY_SIZE * mElementCapacity;
                }
        ...
        void setEx(const TYPE& value, uint32_t pos) {
            if( pos < mTempFileSize/sizeof(TYPE)) {
                ...
                mCurrTableEntriesElement[(pos % (mElementCapacity * ENTRY_SIZE))] = value;
        ...
        bool getEx(TYPE& value, uint32_t pos) const {
            if( pos < mTempFileSize/sizeof(TYPE)) {
                ...
                value = mCurrTableEntriesElement[(pos % (mElementCapacity * ENTRY_SIZE))];
        ...
        bool needNewCurrTable() {
            ...
            if (mCurrTableEntriesElement != NULL && mTempFile != NULL) {
                int ret = fwrite(mCurrTableEntriesElement, sizeof(TYPE) * ENTRY_SIZE, mElementCapacity, mTempFile);
                mTempFileSize += sizeof(TYPE) * ENTRY_SIZE * mElementCapacity;
        ...
        void writeTempFile(MPEG4Writer *writer, uint32_t &nEntries) const {
            ...
            void* tempBuf = malloc(sizeof(TYPE) * ENTRY_SIZE * mElementCapacity);
            while (tempFileSize != 0)
            {
                ALOGD("[%s] fread+, tempFileSize=%zu", mTempFileName.string(), tempFileSize);
                int ret = fread(tempBuf, sizeof(TYPE) * ENTRY_SIZE * mElementCapacity, 1, mTempFile);
                ALOGD("[%s] fread-, ret=%d", mTempFileName.string(), ret);
                writer->write(tempBuf, sizeof(TYPE) * ENTRY_SIZE * mElementCapacity);
                tempFileSize -= sizeof(TYPE) * ENTRY_SIZE * mElementCapacity;
                nEntries -= mElementCapacity;
            }
            free(tempBuf);
        }
...
// for vsdof, video stop spent long time, it will cause audio longer than video.
// When in vsdof mode, stop audio track first.
        if (mAudioStopFirst) {
            status_t status = OK;
            if (mTracks.size() == 2) {
                if (it == mTracks.begin()) {
...
#ifdef MTK_AOSP_ENHANCEMENT
      mStszTableEntries(new ListTableEntries<uint32_t, 1>(TRACK_STSZ_TABLE_BUFFER_SIZE/4)),
#else
...
#ifdef MTK_AOSP_ENHANCEMENT
      mSttsTableEntries(new ListTableEntries<uint32_t, 2>(TRACK_STTS_TABLE_BUFFER_SIZE/4/2)),
#else
