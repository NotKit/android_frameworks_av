...
#ifdef UIBC_SOURCE_SUPPORT
///M:@{
        mUibcGenSrcEnabled = false;
        char val[PROPERTY_VALUE_MAX];
        if (property_get("media.wfd.source.uibc-enabled", val, NULL)) {
            ALOGI("media.wfd.source.uibc-enabled:%s", val);
            if (val[0] == '1'){
                mUibcGenSrcEnabled = true;
            } else {
                mUibcGenSrcEnabled = false;
            }
        }
        if (mUibcGenSrcEnabled) {
...
status_t WifiDisplaySource::onReceiveM3Response(
    ...
    if (params->findParameter(WFD_UIBC_CAPABILITY, &uibcCapability)) {
        mUibcServerHandler->init((mWfdFlags & kSigmaTest) == kSigmaTest);
        mUibcServerHandler->parseRemoteCapabilities(uibcCapability);
        if (mUibcServerHandler->isUibcSupported() && mUibcGenSrcEnabled) {
...
void WifiDisplaySource::onReceiveTestData(const sp<AMessage> &msg) {
    ...
    // For HDCP
        ...
        response = "OK,\r\n";
        if (mUibcServerHandler != NULL) {
            mUibcServerHandler->setTestMode(true);
        }
...
status_t WifiDisplaySource::setBitrateControl(int bitrate) {
