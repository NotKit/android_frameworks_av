...
//can be used for all class defined here
int g_log_enable = 0;
...
FileCache::FileCache(int fd,
    ...
    if (filename.find("unexpected type for ") == -1 && filename.find("couldn't open ") == -1 ) {
        ...
        if (mDupFd == -1 && g_log_enable){
            SLOGE("Open dupFd fail for file %s,strerror(%d)=%s", filename.c_str(),errno,strerror(errno));
        }
    }
#endif
}
...
FileSourceProxy::FileSourceProxy()
    ...
    g_log_enable = 1;
#else
    g_log_enable = __android_log_is_loggable(ANDROID_LOG_DEBUG,"FSProxy_LOG",ANDROID_LOG_INFO);
#endif
    ALOGD("LOG enable = %d",g_log_enable);
}
