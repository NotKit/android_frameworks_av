...
#ifdef MTK_AOSP_ENHANCEMENT
        init();
        if (!(ARTSPConnection::ParseURL(mSessionURL.c_str(), &host, &port, &path, &user, &pass))) {
...
#ifdef MTK_AOSP_ENHANCEMENT
                    ALOGI("'setu',trackIndex=%zu",trackIndex);
                    if (mTracks.size() == 0) {
                        ALOGW("SETUP %zu done after abor", trackIndex);
                        break;
                    }
#endif // #ifdef MTK_AOSP_ENHANCEMENT
...
#ifdef MTK_AOSP_ENHANCEMENT
                        ALOGI("'setu',create 'accu' notify with trackIndex=%zu for ARTPCon",trackIndex);
                        if (i < 0) {
                            // server maybe give wrong header "trensport"
                            ALOGW("get header 'transport' failed, i(%zd) < 0, use 'trensport' to get again", i);
...
#ifdef MTK_AOSP_ENHANCEMENT
            // enable SR timestamp
            ALOGI("no range, using SR ntp, using last play time %lld", (long long)mLastPlayTimeUs);
...
private:
    struct TrackInfo {
        ...
        void setTimestamp(uint32_t rtpTime, int64_t ntpTimeUs, bool fake = false) {
            ...
            mUseTrackNTP = fake;
            ALOGI("setTimestamp rtp %d, ntp %lld, track %d", mRTPAnchor, (long long)mNTPAnchorUs, mUseTrackNTP);
        }

        void resetTimestamp() {
            ALOGI("resetTimestamp rtp %d, ntp %lld, track %d", mRTPAnchor, (long long)mNTPAnchorUs, mUseTrackNTP);
            mRTPAnchor = 0;
            mNTPAnchorUs = -1;
            mUseTrackNTP = false;
            mFirstAccessUnit = true;
    ...
    void setupTrack(size_t index) {
        ...
        ALOGI("setupTrack,mTracks.size()=%zu",mTracks.size());
#else
        mTracks.push(TrackInfo());
...
#ifdef MTK_AOSP_ENHANCEMENT
        ALOGI("postQueueEOS for %zu: %d", trackIndex, finalResult);
#endif
        sp<AMessage> msg = mNotify->dup();
    ...
    void postQueueSeekDiscontinuity(size_t trackIndex) {
#ifdef MTK_AOSP_ENHANCEMENT
        if (mIsLegacyMode) {
            ALOGI("flush track %zu", trackIndex);
            TrackInfo *info = &mTracks.editItemAt(trackIndex);
        ...
        ALOGI("postQueueSeekDiscontinuity flush track %zu", trackIndex);
#endif
        sp<AMessage> msg = mNotify->dup();
...
public:
    ...
    void switchingTCP() {
        ...
        if (mLastSeekTimeTime != -1) {
            ...
            ALOGI("Switching to TCP, seek to mLastSeekTimeTime=%lld",(long long)mLastSeekTimeTime);
        } else {
            sp<AMessage> msg = new AMessage('resu', this);
            msg->post();
        }
    ...
    void onStreamTomeOut(TrackInfo *track, size_t trackIndex) {
        ALOGI("MyHandler: dead track #%zu", trackIndex);
        if (!mReceivedFirstRTPPacket && (!mTryTCPInterleaving || mSwitchingTCP)) {
            ALOGI("don't kill track #%zu which is dead before trying TCP", trackIndex);
            return;
        }
         ...
         ********************************************************************************************/
        ALOGI("accu,track->mLastMediaTimeUs=%lld,mNPTMode=%d",(long long)track->mLastMediaTimeUs,mNPTMode);
        if((track->mPlayStartTime * 1E6 > track->mLastMediaTimeUs) && mNPTMode){
            track->mLastMediaTimeUs += ARTPSource::kAccessUnitTimeoutUs;
            ALOGI("accu,track->mLastMediaTimeUs=%lld",(long long)track->mLastMediaTimeUs);
            if(track->mPlayStartTime* 1E6 > track->mLastMediaTimeUs){
                ALOGI("accu,track->mPlayStartTime=%f,track->mLastMediaTimeUs=%lld",track->mPlayStartTime,(long long)track->mLastMediaTimeUs);
                return;
            }
        }
        ...
        ALOGI("mLastMediaTimeUs %lld %lld, duration %lld, mode %d, live %d",
             (long long)mLastMediaTimeUs, (long long)track->mLastMediaTimeUs, (long long)tmp, mNPTMode, !mSeekable);
    ...
    void parsePlayResponse_l(Vector<uint32_t> rtpTimes, Vector<uint32_t> rtpSeqs, Vector<int32_t> rtpSockets, float npt1, float npt2) {
        if (mTracks.size() != rtpTimes.size()) {
            ...
            if((int64_t)(npt1 * 1E6) >= mMediaAnchorUs || mSeekPending) {
                ...
                ALOGI("[rtsp](int64_t)(npt1 * 1E6) %lld < mMediaAnchorUs %lld",(long long)(npt1 * 1E6),(long long)mMediaAnchorUs);
    ...
    bool skipMultiStreams(sp<APacketSource> source, size_t index) {
        ...
        if (!strncasecmp(mime, "video/", 6)) {
            if (mHaveVideo) {
                ALOGW("Skip multiple video stream. Ignoring track #%zu.",
                        index);
                skip = true;
            } else {
                mHaveVideo = true;
            ...
            if (mHaveAudio) {
                ALOGW("Skip multiple audio stream. Ignoring track #%zu.",
                        index);
                skip = true;
            } else {
                mHaveAudio = true;
            ...
            ALOGW("Unsupported format %s. Ignoring track #%zu.", mime,
                    index);
            skip = true;
        }
     ...
     info->mIsEOSByError = false;
        ...
        if(mSessionDesc->findAttribute(index, "a=range", &play_range)){
            ...
            if (ASessionDescription::parseNTPRange(play_range.c_str(), &range1, &range2)){
                ...
                ALOGI("track %zu,a=range:%.2f-%.2f",index,range1,range2);
            }
        }
    }
    ...
    void  supBitRateAdap(TrackInfo *info, AString &request, size_t &index, AString &trackURL) {
        ...
        ALOGI("NADU Frequence =%lu",info->mNADUFreq);
    ...
    int64_t getMediaTimeUs(TrackInfo *track, const sp<ABuffer> accessUnit, uint32_t rtpTime, int64_t &ntpTimeUs) {
        if (((int32_t)(rtpTime - track->mRTPAnchor)) > 0x40000000) {
            ALOGI("force update rtp anchor to %d %lld %d", rtpTime, (long long)ntpTimeUs, track->mUseTrackNTP);
            ...
            if (!mNPTMode && !track->mUseTrackNTP) {
                ...
                if (diff > kMaxSRNTPDiff) {
                    ALOGW("NTP big difference %lld vs %lld, enable track NTP mode",
                            (long long)mNTPAnchorUs, (long long)track->mFirstAccessUnitNTP);
                    track->mUseTrackNTP = true;
                }
            }
            ALOGI("first segment unit ntpTimeUs=0x%016llx rtpTime=%u seq=%d diff=%llx",
                    (long long)ntpTimeUs, rtpTime, accessUnit->int32Data(), (long long)diff);
        }

        ALOGV("times %lld %lld %lld %lld", (long long)ntpTimeUs,
            (long long)track->mFirstAccessUnitNTP, (long long)mNTPAnchorUs, (long long)mMediaAnchorUs);
    ...
    void flushPackets(size_t trackIndex) {
        ...
        if(!(trackInfo->mPackets.empty())){
            ...
            ALOGI("flush track %zu mPackets Error:begin %d size %zu",trackIndex,begin->int32Data(),trackInfo->mPackets.size());
    ...
    void preSeek(int64_t timeUs) {
        sp<AMessage> msg = new AMessage('prse', this);
        msg->setInt64("time", timeUs);
        msg->post();
    }
...
// mtk80902: little bit ugly..
    ...
    for (size_t i = 0; i < mTracks.size(); ++i) {
        TrackInfo *track = &mTracks.editItemAt(i);
        ALOGD("set source params: track %zu's source: %p", i, track->mPacketSource.get());
    ...
    status_t _seek(int64_t timeUs) {
        ...
        if (!mPlaySent) {
            if (mServerInfo.find("MDN_HWPSS") != -1) {
                ...
                sp<AMessage> reply = new AMessage('nopl', this);
                reply->setInt64("time", timeUs);
                _play(0, reply);
            }
        }
