...
#ifdef MTK_AOSP_ENHANCEMENT
        } else if (mHasAudio) { //mtk has special setting for Audio track
#else
...
status_t MPEG4Extractor::parseChunk(off64_t *offset, int depth) {
    ...
    switch(chunk_type) {
        ...
        case FOURCC('w', 'a', 'v', 'e'):
        {
            ...
            *offset = data_offset;
            while (*offset < stop_offset) {
#ifdef MTK_AOSP_ENHANCEMENT //for .mov file
...
#ifdef MTK_AOSP_ENHANCEMENT
            if (mIsQT){ // change to work on for all codecs contained in QT file
#else
            if (mIsQT && chunk_type == FOURCC('m', 'p', '4', 'a')) {
#endif
                if (version == 1) {
                    ...
                    *offset += 16;
                    ...
                    uint64_t tmp = U64_AT(&v2buffer[4]);
                    ...
                    ALOGI("sampleRate:%f %d,num_channels=%d", *pSampleRate, sample_rate,num_channels);

                    //the following google solution for sample_rate will be 0
                    //and have no idear why google mark the following code
                    //if google add right solution , we need change to use google's solution
#endif
            ...
            *offset += chunk_size;
            ...
            if (mPath.size() >= 2
                ...
                if (esds.getObjectTypeIndication(&objectTypeIndication) == OK) {
                    ...
                    if(codec_specific_data_size < 4){
                       ALOGD("esds getCodecSpecificInfo,csd with invalid size");
                       mLastTrack->meta->setInt32(kKeyCodecInfoIsInFirstFrame, true);
                    }else{
