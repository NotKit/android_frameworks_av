...
SoftAAC2::SoftAAC2(
    ...
    if (property_get("aac.decoder.dump", value, NULL)) {
        mDecoderDump = atoi(value);
    } else {
        mDecoderDump = 0;
    }
#endif
    ...
    while ((!inQueue.empty() || mEndOfInput) && !outQueue.empty()) {
        if (!inQueue.empty()) {
...
#ifdef MTK_AOSP_ENHANCEMENT
            if (mEndOfInput) ALOGI("input eos, length=%d", inHeader->nFilledLen);
#endif
...
#ifdef MTK_AOSP_ENHANCEMENT
            if (inHeader->nFilledLen == 0 && (getBytesInDecoder() == 0 || nDecodingNotEnoughBits)) {
#else
...
#ifdef MTK_AOSP_ENHANCEMENT
            if (mIsADTS && inHeader->nFilledLen == 0 && getBytesInDecoder() != 0) {
                ALOGI("the current adts frame has not decoded complete");
                // skip to fill decoder, just aacDecoder_DecoderFrame() again.
                inBufferLength[0] = 0;
            } else
#endif
            if (mIsADTS) {
...
#ifdef MTK_AOSP_ENHANCEMENT
                ALOGV("input length=%u", inBufferLength[0]);
#endif
            ...
            if (inBufferLength[0] > 0) {
...
#ifdef MTK_AOSP_ENHANCEMENT
                dumpDecoderIn(inBuffer[0], inBufferLength[0]);
#endif
                ...
                ALOGV("aacDecoder_Fill length=%u", inBufferUsedLength);
            ...
            do {
...
#ifdef MTK_AOSP_ENHANCEMENT
                int32_t bytesPerChannel = mStreamInfo->frameSize * sizeof(int16_t);
                for (int i = 0; i < mStreamInfo->numChannels; i++) {
                    // dump decoder data by channel
                    dumpDecoderOut(tmpOutBuffer + mStreamInfo->frameSize * i, bytesPerChannel, i);
                }
#endif
...
#ifdef MTK_AOSP_ENHANCEMENT
                ALOGV("decoded length=%d", numConsumed);
#endif
                if (decoderErr != AAC_DEC_OK) {
                ...
                if (!mStreamInfo->sampleRate || !mStreamInfo->numChannels) {
                    ...
                    if (inHeader && inHeader->nFilledLen == 0) {
                        ...
                        inInfo = NULL;
                        notifyEmptyBufferDone(inHeader);
                        ALOGV("EBD when nFilledLen=0 & getBytesInDecoder()==0");
                        inHeader = NULL;
                    }
                    return;
                }
...
#ifdef MTK_AOSP_ENHANCEMENT
            ALOGI("discard %d samples at the beginning(outputDelay = %d, compensated = %d)",
                  discarded, outputDelay, mOutputDelayCompensated);
#endif
...
int SoftAAC2::getBytesInDecoder() {
    ...
    if (mDecoderInternelBufferSize <= freeBytesInDecoder) {
        return 0;
    } else {
        int diff = mDecoderInternelBufferSize - freeBytesInDecoder;
        ALOGI("diff(%d) > 0, mDecoderInternelBufferSize(%u), freeBytesInDecoder(%u)",
...
void SoftAAC2::handleAviWithRawAAC() {
    ALOGD("mIsRawAac(%d), mIsErrorCsdHandled(%d), mInputBufferCount(%zu)",
...
void SoftAAC2::dumpDecoderIn(void *inBuffer, UINT size) {
    if (mDecoderDump) {
        FILE *fpin = fopen("/sdcard/aacdecoder_in.bs", "ab");
        if (fpin) {
            fwrite(inBuffer, 1, size, fpin);
            fclose(fpin);
        } else {
            ALOGI("failed to open /sdcard/aacdecoder_in.bs");
        }
    }
}

void SoftAAC2::dumpDecoderOut(void *inBuffer, int32_t size, int i) {
    if (mDecoderDump) {
        char outFile[128] = {0};
        sprintf(outFile, "/sdcard/aacdecoder_out_%d.pcm", i);
        /*
           aacdecoder_outl.pcm
           aacdecoder_outr.pcm
           aacdecoder_outc.pcm
           aacdecoder_outlfe.pcm
           aacdecoder_outls.pcm
           aacdecoder_outrs.pcm
         */
        FILE *fpout = fopen(outFile, "ab");
        if (fpout) {
            fwrite(inBuffer, 1, size, fpout);
            fclose(fpout);
        } else {
            ALOGI("failed to open /sdcard/aacdecoder_out.pcm");
        }
    }
}
