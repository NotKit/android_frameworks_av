...
private:
    ...
    /// VideoTransdecoder will use OMXCodec to decode mkv file
    /// In OMXCodec, it will generate many thread to read parser.
    /// So it will caused multi-thread read parser. It will caused memory
    /// corruption issue. So MatroskaSource's public function will add
    /// autoLock to prevent this issue.
    /// ALPS02464086/ALPS02463155/ALPS02460717/ALPS02458002
    Mutex mLock;
#endif
...
private:
    status_t mergeMultiFrame2OneFrame(const mkvparser::Block *block, int64_t timeUs);
    status_t readBlockWithContentEncoding(const mkvparser::Block *block,  int64_t timeUs);
#endif
};
...
MatroskaSource::MatroskaSource(
    ...
    : mExtractor(extractor),
      ...
      mNALSizeLen(-1) {
        ...
        if (!strcmp("V_MS/VFW/FOURCC", CodecId)) mIsFromFFmpeg = true;
...
bool BlockIterator::eos() const {
    ...
    return (mCluster == NULL || mCluster->EOS()) ||
                ((mEosType == ERRJUMP) && (mTrackType == 17));
#else
    return mCluster == NULL || mCluster->EOS();
#endif
}
...
status_t MatroskaSource::readBlock() {
    ...
    if (mType == MP2_3) {
        err = mergeMultiFrame2OneFrame(block, timeUs);
    } else {
        err = readBlockWithContentEncoding(block, timeUs);
    }
    if (OK != err) {
        ALOGE("read block error, mType = %d, err = %d", mType, err);
        return err;
    }
#else
...
status_t MatroskaSource::read(
    ...
    *out = buffer;
    ...
    return OK;
        ...
        size_t NALsize = 0;
        if (frame->range_length() >=4 && *(data+0) == 0x00 && *(data+1) == 0x00
            ...
            int tail = 4;
            while(tail <= size - 4) {
                if ((*(data+tail+0) == 0x00 && *(data+tail+1) == 0x00 &&
                    *(data+tail+2) == 0x00 && *(data+tail+3) == 0x01) || tail == size -4) {
                    int nalsize = 0;
                    if (tail == size -4) {
                        nalsize = size;
                    } else {
...
void MatroskaExtractor::addTracks() {
                ...
                if (!strcmp("V_MPEG4/ISO/AVC", codecID)) {
...
#ifdef MTK_AOSP_ENHANCEMENT
                else if (!strcmp("V_MPEG4/ISO/ASP", codecID) || !strcmp("V_MPEG4/ISO/SP", codecID)) {
                    ...
                    if (NULL == codecPrivate ||codecPrivateSize < 20) {
                        ...
                        if (!strcmp(mime, MEDIA_MIMETYPE_VIDEO_AVC)) {
                            ...
                            uint16_t divx3_width = 0;
                            uint16_t divx3_height = 0;
...
#ifdef MTK_AOSP_ENHANCEMENT
    #if defined (MTK_AUDIO_DDPLUS_SUPPORT)  && defined (MTK_MKV_PLAYBACK_ENHANCEMENT)
    ...
    #if defined (MTK_AUDIO_RAW_SUPPORT) && defined(MTK_MKV_PLAYBACK_ENHANCEMENT)
...
status_t MatroskaSource::mergeMultiFrame2OneFrame(const mkvparser::Block *block, int64_t timeUs) {
    uint64_t size = block->GetDataSize();  // the size of all the frame sum. frame1 +......+ frame(n)
    size_t frameCount = block->GetFrameCount();
    MediaBuffer *bigbuf = new MediaBuffer(size+frameCount*mTrackContentAddDataSize);
    uint64_t buf_offset = 0;
    for (size_t i = 0; i < frameCount; ++i) {
        const mkvparser::Block::Frame &frame = block->GetFrame(i);
        MediaBuffer *mbuf = new MediaBuffer(frame.len);
        status_t err = frame.Read(mExtractor->mReader, (unsigned char *)mbuf->data());
        if (err == OK &&
            mExtractor->mIsWebm &&
            mExtractor->mTracks.itemAt(mTrackIndex).mEncrypted) {
            err = setWebmBlockCryptoInfo(mbuf);
    ...
    if (buf_offset != (size+frameCount*mTrackContentAddDataSize)) {
        unsigned long long lostNum = buf_offset - (size + frameCount*mTrackContentAddDataSize);
        ALOGD("mp3 data count failed,we lost %llu number data ", lostNum);
        return ERROR_IO;
    }
...
status_t MatroskaSource::readBlockWithContentEncoding(const mkvparser::Block *block, int64_t timeUs) {
    size_t frameCount = block->GetFrameCount();
    for (size_t i = 0; i < frameCount; ++i) {
        const mkvparser::Block::Frame &frame = block->GetFrame(i);
        ...
        status_t err = frame.Read(mExtractor->mReader, (unsigned char *)mbuf->data()+   mTrackContentAddDataSize);
        if (err == OK &&
            mExtractor->mIsWebm &&
            mExtractor->mTracks.itemAt(mTrackIndex).mEncrypted) {
            err = setWebmBlockCryptoInfo(mbuf);
