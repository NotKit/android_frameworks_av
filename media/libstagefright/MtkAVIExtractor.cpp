...
sp<MetaData> MtkAVIExtractor::getTrackMetaData(
    ...
    if ((flags & kIncludeExtensiveMetaData)
        ...
        if (!strncasecmp("video/", mime, 6)) {
            ...
            LOGI("thumbnail index %d, time %lld", track->mThumbNailIndex, (long long)dts);
        }
    }
...
status_t MtkAVIExtractor::addSample(size_t id, struct MtkAVISample *s) {
    if (id >= mTracks.size()) {
        LOGW("skip invalid index for chunk %zu", id);
    ...
    if (mFileSize < s->offset) {
        // for ALPS01489456,  the last of index table is invalid
        LOGE("addSample--invalid offset,ignore it. s.offset:%lld", (long long)s->offset);
        return OK;
    }
...
status_t MtkAVIExtractor::parseDataChunk(MtkAVIOffT pos, int ID, uint32_t size, int sync) {
    ...
    //LOGV("ID " FORMATFOURCC " at pos 0x%08llx %d", PRINTFOURCC(ID), (long long)pos, gettid());//must mark , may lead NE
    if (size == 0 && ID == 0) {
    ...
    switch(type) {
        ...
        default:
            if(logtimes<5 || logtimes%100 == 0)
                LOGW("skip chunk at pos %lld " FORMATFOURCC, (long long)pos, PRINTFOURCC(ID));
             logtimes++;
             return BAD_VALUE;
    }
    return OK;
...
status_t MtkAVIExtractor::parseMOVIMore(bool full) {
    ...
    LOGI("parse movi chunks,mMOVIOffset:0x%x to setup index from 0x%llx to 0x%llx, mFileSize:0x%llx. full=%d, 0x%zd",
    ...
    while(pos < oldEnd) {
        ...
        if (s != kSizeOfListHeader) {
            if (s != kSizeOfChunkHeader || list.size != 0) {
                LOGE("failed to read header at 0x%08llx, stop parsing", (long long)pos);
            }
            break;
        }
        LOGV(" at pos 0x%08llx,ID: %d size:%u", (long long)pos,list.ID,list.size);

        err = OK;
        MtkAVIOffT end = pos + EVEN(list.size) + kSizeOfSkipHeader;
        if (end > oldEnd) {
            LOGW("skip last incomplete chunk/list at %llx-%llx", (long long)pos, (long long)end);
            break;
        }
            ...
            if (list.type == BFOURCC('r', 'e', 'c', ' ')) {
                ...
                while (pos < end) {
                    ...
                    if (s != kSizeOfListHeader && s != kSizeOfChunkHeader) {
                        LOGE("failed to read header at 0x%llx", (long long)pos);
                        break;
                    }
                    pos += kSizeOfChunkHeader;
                    MtkAVIOffT chunkEnd = pos + EVEN(chunk.size);
                    if (chunkEnd > end) {
                        LOGW("skip last incomplete chunk in rec at %llx-%llx", (long long)pos, (long long)chunkEnd);
                        break;
                    }
        ...
        if (metadataKey != 0) {
            ...
            if (size > 4096) {
                ...
                if (mDataSource->readAt(pos, value->data(), (size+1)) != (ssize_t)(size+1)) {
                    LOGW("read metadata error at %llx", (long long)pos);
                } else {
                    value->data()[size] = 0;
...
status_t MtkAVIExtractor::parseAVIH(MtkAVIOffT pos, MtkAVIOffT end) {
    ...
    if (end - pos < sz) {
        LOGE("sizeof avih is not correct: %lld < %lld", (long long)(end - pos), (long long)sz);
        return ERROR_MALFORMED;
    }

    ssize_t s = mDataSource->readAt(pos, (char*)&header, sz);
    if (s != sz) {
        LOGE("failed to read header at 0x%08llx", (long long)pos);
        return ERROR_IO;
    }
...
status_t MtkAVIExtractor::parseSTRL(MtkAVIOffT pos, MtkAVIOffT oldEnd, int index) {
    ...
    while(pos < oldEnd) {
        ...
        LOGV("ID " FORMATFOURCC " at pos 0x%08llx", PRINTFOURCC(list.ID), (long long)pos);
            ...
            switch(list.ID) {
                ...
                case BFOURCC('s','t','r','h'):
                    ...
                    if( parseSTRH(pos, end, source)!= OK )//fixed ALPS01487202
                    {
                        ...
                        LOGE("parse strh fail, remove track, %zu", mTracks.size());
                    }
                    break;
    ...
    if (pos > oldEnd || !strhFound || !strfFound) {
        LOGE("strl of stream %d seems to be corrupted: end at 0x%08llx, "
...
status_t MtkAVIExtractor::parseChunkIndex(MtkAVIOffT pos, MtkAVIOffT end, sp<MtkAVISource> source,
    ...
    LOGV("parse chunk index %llx %llx %p", (long long)pos, (long long)end, pHeader);
    struct MtkAVIIndexChunk header;
    if (pHeader == NULL) {
        ssize_t sz = kSizeOfAVIIndexChunk;
        if (end - pos < sz) {
            LOGW("sizeof indx is not correct: %lld < %lld", (long long)(end - pos), (long long)sz);
            return ERROR_MALFORMED;
        }

        ssize_t s = mDataSource->readAt(pos, (char*)&header, sz);
        if (s != sz) {
            LOGW("failed to read header at 0x%08llx", (long long)pos);
            return ERROR_IO;
        }
    ...
    if (end - pos < numbers * (int)sizeof(aviStdIndexEntry)) {
        LOGW("not enough data for index %llx - %llx < %d",
    ...
    while(i < numbers) {
        ...
        if (mDataSource->readAt(pos + i * kSizeOfAVIStdIndexEntry,
            ...
            LOGW("failed to read chunk entry at 0x%08llx", (long long)pos);
            return ERROR_IO;
        }
        ...
        if((entry.offset + baseOffset+(entry.size & 0x7fffffff)) > mFileSize){
            LOGI("break addSample! offset %d, size %d,mFileSize %lld",
...
status_t MtkAVIExtractor::parseINDX(MtkAVIOffT pos, MtkAVIOffT end, sp<MtkAVISource> source) {
    ...
    if (tmpOffset != baseOffset) {
        LOGW("TODO support 64 bits file %llx", (long long)tmpOffset);
        return ERROR_UNSUPPORTED;
    }
    ...
    if (header.indexType == kKeyAVIIndexOfIndexes) {
        ...
        if (end - pos < numbers * (int)sizeof(aviStdIndexEntry)) {
            LOGW("not enough data for index %llx - %llx < %zu",
        ...
        for(i = 0; i < numbers; i++) {
            ...
            if (mDataSource->readAt(pos, (char*)&entry, kSizeOfAVISuperIndexEntry)
                ...
                LOGW("failed to read index entry at 0x%08llx", (long long)pos);
                return ERROR_IO;
            }
...
status_t MtkAVIExtractor::parseSTRN(MtkAVIOffT off, MtkAVIOffT end, sp<MtkAVISource> source) {
...

    {
        ...
        if (s != end - off) {
            LOGE("parseSTRN failed to read header at 0x%08llx", (long long)off);
            return ERROR_IO;
        }
        if (!strncmp(ptr,"Subtitle",strlen("Subtitle"))){
            ...
            LOGW("GET subtitle chunk %zu",strlen("Subtitle - "));
        }
            ...
            LOGW("GET vids chunk %zu",strlen("Video - "));
        }
            ...
            LOGW("GET Aud chunk %zu",strlen("Audio - "));
        }
...
status_t MtkAVIExtractor::parseSTRF(MtkAVIOffT pos, MtkAVIOffT end, sp<MtkAVISource> source) {
    ...
    if (end - pos < sz) {
        LOGE("sizeof strf is not correct: %lld < %lld", (long long)(end - pos), (long long)sz);
        return ERROR_MALFORMED;
    }
    ...
    if (end - pos > sz) {
        ...
        if (source->mIsAudio && leftBytes > 2) {
            if (mDataSource->readAt(pos + offset, (char*)&size, 2) != 2) {
                LOGE("failed to read header at 0x%08llx", (long long)(pos + offset));
                return ERROR_IO;
            }
            if (size <= leftBytes) {
                valid = true;
        ...
        if (valid) {
            ...
            LOGI("get extra codec data %lld bytes", (long long)size);
        } else {
            LOGW("skip invalid codec data %lld bytes", (long long)leftBytes);
        }
    }
    ...
    if (source->mIsVideo) {
        ...
        if (source->mIsAVC) {
            ...
            if(source->AVCStyle == SIZE_NAL_TYPE ) {
                ...
                ALOGI("AVCSizeLength = %lld", (long long)source->AVCSizeLength);
            }
        ...
        if (!strcmp(mime, MEDIA_MIMETYPE_AUDIO_MS_ADPCM)
            ...
            if (source->mSize != 0) {
                ...
                LOGI("strfData size is %zu", source->strfData->size());
            } else {
...
status_t MtkAVIExtractor::parseSTRH(MtkAVIOffT pos, MtkAVIOffT end, sp<MtkAVISource> source) {
    ...
    if (end - pos < sz) {
        LOGE("sizeof strh is not correct: %lld < %lld", (long long)(end - pos), (long long)sz);
        return ERROR_MALFORMED;
    }
...
status_t MtkAVIExtractor::parseHDRL(MtkAVIOffT pos, MtkAVIOffT oldEnd, MtkAVIOffT &endOffsCRC) {
    ...
    while(pos < oldEnd) {
        ...
        LOGE("ID " FORMATFOURCC " at pos 0x%llx, list.size:0x%x,0x%x",
...
status_t MtkAVIExtractor::parseIDX1(MtkAVIOffT pos, MtkAVIOffT end) {
    ...
    if(pageSize > 0 && numPages > 0){
        if (size_t(numPages) < SIZE_MAX / size_t(pageSize)) {
            ...
            LOGD("physMem:%zu,samlpenumbers:%d", maxMem, numbers);
            if( ((numbers*18) > (int)(maxMem*0.4)) || (numbers*9) > (100*1024*1024))
                return ERROR_MALFORMED;
        }
    ...
    while(i < numbers) {
        ...
        if (mDataSource->readAt(pos + i * kSizeOfAVIOldIndexEntry,
            ...
            LOGE("failed to read old index entry at 0x%08llx", (long long)(pos + i * kSizeOfAVIOldIndexEntry));
            return ERROR_IO;
        }
...
status_t MtkAVIExtractor::checkCapability() {
    for(size_t i=0; i<mTracks.size(); ++i) {
        ...
        if (err == ERROR_UNSUPPORTED) {
            ...
            LOGI("finished sample number %zu", source->mSampleSizes.size());
        }
    }
...
status_t MtkAVIExtractor::parseFirstRIFF() {
    ...
    if (size > kAVIMaxRIFFSize) {
        LOGI("size of RIFF is out of spec %zd > %d", size, kAVIMaxRIFFSize);
        //return ERROR_MALFORMED;
    }
    ...
    if (fSize <= 0) {
        LOGE("abnormal file size %lld. out of 32 bit? not supported yet", (long long)fSize);
        return ERROR_UNSUPPORTED;
    }else{
        LOGE("mFileSize: 0x%llx.", (long long)fSize);
    }

    if (size + kSizeOfSkipHeader > fSize) {
        // TODO do our best to support chunked file
        LOGW("file seems to be chunked (%zd + %d) > %lld", size, kSizeOfSkipHeader, (long long)fSize);
    }
    ...
    int32_t times=0;
    LOGI("size of RIFF  0x%zx -- 0x%x, pos:0x%llx, oldEnd:0x%llx",
    ...
    while(pos < oldEnd) {
        ...
        if (pos > fSize) {
            LOGW("chunked file? %llx > %llx", (long long)pos, (long long)fSize);
            break;
        }
        ...
        LOGV("parseFirstRIFF ID " FORMATFOURCC " at pos 0x%llx,list.size:0x%x,0x%x",
            PRINTFOURCC(list.ID), (long long)pos, list.size, EVEN(list.size));
        if ( 0x0 == list.ID ){
        ...
        if (list.ID == BFOURCC('L','I','S','T')) {
            ...
            switch (list.type) {
                case BFOURCC('h','d','r','l'):
                    ...
                    LOGE("found hdrl over,end:0x%llx,endOffsCRC:0x%llx", (long long)end, (long long)endOffsCRC);
    ...
    if (!mHasIndex) {
        for(size_t i=0; i<mTracks.size(); ++i) {
            ...
            LOGI("update duration for broken file: %lldus", (long long)time);
        }
    }
...
status_t MtkAVISource::read(
    ...
    *out = NULL;
    ...
    if (options && options->getSeekTo(&seekTimeUs, &mode)) {
        LOGI("seekTimeUs=%lld, seekMode=%d, broken=%d", (long long)seekTimeUs, mode, mBrokenIndex);
        mIsMultiple = false;
        mLastBufferOffset = 0;
        mCurrentPTSDelta = 0;
            ...
            while(low < high) {
                ...
                if ((mAvgBytesPerSec >0) && isRawAACData) {
                    ...
                    dts = (int64_t)(totalSize) * 1000000LL / mAvgBytesPerSec ;
                        LOGD("[Jimmy] ln(%d), CBR seek to dts(%lld), index(%d), totalSize(%d), mScale(%d), mRate(%d), mSampleSize(%d)",
    ...
    if (newBuffer) {
        ...
        if((size + mBuffer->range_offset() ) > mBuffer->size()){
            LOGE("mLastBufferOffset:%d, size:%zu, range_offset:%zu", mLastBufferOffset, size, mBuffer->range_offset());
            mBuffer->release();
            mBuffer = NULL;
            return ERROR_MALFORMED;
        }
        ...
        if (num_bytes_read < (ssize_t)size) {
            mBuffer->release();
            mBuffer = NULL;
            LOGE("failed to read data at 0x%08llx", (long long)offset);
            return ERROR_IO;
        }
        ...
        if (isRawAACData) {
            ...
            if (bVBRWise)
            {
                // VBR-like
            dts = (int64_t)(mCurrentSampleIndex - 1) * 1000000LL * mScale / mRate;
                LOGD("[Jimmy] Audio ln(%d), VBR dts(%lld), mCurrentSampleIndex(%d), mScale(%d), mRate(%d), mSampleSize(%d)",
            ...
            else
            {
                // CBR-like
                dts = (int64_t)(totalSize) * 1000000LL / mAvgBytesPerSec ;
                LOGD("[Jimmy] Audio ln(%d), CBR dts(%lld), mCurrentSampleIndex(%d), mScale(%d), mRate(%d), mSampleSize(%d)",
                    __LINE__, (long long)dts, mCurrentSampleIndex, mScale, mRate, mSampleSize);
            }
        } else {
            dts = (int64_t)(totalSize) * 1000000LL * mScale / mRate / mSampleSize;
        ...
        LOGV("v=%d, dts=%lld off=%llx s=%zx totalSize=%x tid=%d",
    ...
    if(mMP3Header < 0 && afterseek && (dts<seekTimeUs && seekTimeUs-dts>100000))
    {
        NeedRefinePos = true;
        mCurrentDTS = dts;
        LOGI("seekTimeUs-dts>100000!!!! dts=%lld,NeedRefinePos=%d", (long long)dts, NeedRefinePos);
    }
    ...
    do{
        ...
        LOGV("mp3 start %d header %x frameSize %zu length %d bitrate %d", start, header, frameSize, length, bitrate);
        ...
        if (start >= 0 && ret) {
            ...
            fwrite(tmp->data() + tmp->range_offset(), 1, tmp->range_length(), fp);
                ...
                LOGW("send incomplete MP3 frame %d < %zu", length - start, frameSize);
        ...
        if(NeedRefinePos)
        {
             ...
             mBuffer->set_range(
                ...
                LOGI("refine pos: NeedRefinePos=%d,mCurrentDTS=%lld, begin=%zu,end=%zu",
    ...
    while(start < (int)mSampleSizes.size()) {
        ...
        while(i + 3 < size) {
            ...
            if (get_mp3_frame_size(header1, &frame_size, NULL, NULL, NULL)) {
                ...
                LOGI("possible header %x at %x size %zx, test %x", header1, offset + i, frame_size, header2);
...
status_t MtkAVISource::findRealKeyframe() {
    ...
    int AVCPos = 0;
    int offsetStart = 0;
    LOGW("findRealKeyframe mSampleSizes.size:%zu", mSampleSizes.size());
    while(i < mSampleSizes.size() && (i <= 10))
    {
        ...
        LOGW("findRealKeyframe ,i:%zu ,size:%d ,offset:%lld ", i, size, (long long)offset);
        sp<ABuffer> buffer = new ABuffer(size);
        if (mDataSource->readAt(offset, (uint8_t*)buffer->data(), size) != size) {
            LOGE("failed to read data at 0x%08llx", (long long)offset);
            return ERROR_IO;
        ...
        if (!found) {
            ...
            if (mIsAVC){
                ...
                LOGW("strf size of %llu:, AVCPos: %d, dumping first %d",
             ...
             if(mIsAVC)
             {
                ...
                LOGE("FindAVCSPS, mLevel:%x", mLevel);
                if(/*0xff == mLevel && */!makeSpecData){
                    i++;
                }else{
                    LOGE("find the real I Frame on the chunk, i:%zu", i);
                    break;
                }
             }
        }
...
status_t MtkAVISource::generateCodecData(bool full) {
    ...
    ALOGV("generate codec info , avc style is %d", SIZE_NAL_TYPE);
    ...
    if (!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_AVC)
        ...
        if (strfData != NULL && mIsAVC) {
            ...
            if (nalStartAt(strfData->data(), strfData->size(), &prefixLen) < 0) {
                if (nalStartAt2(strfData->data(), strfData->size(), &prefixLen) <= 0){
                    ...
                    LOGW("discard bad strf size of %zu: dumping first %d", strfData->size(), s);
                    for(int i = 0; i < s; ++i) {
        ...
        if (mSampleSizes.size() > 0) {
            ...
            if (!found) {
                ...
                if (mIsAVC) {
                    s = (strfData->size() - AVCPos) > 256 ?  256 : (strfData->size() - AVCPos);
                    LOGW("strf size of %llu: dumping first %d", (long long)(strfData->size() - AVCPos), s);
                } else {
                    s = strfData->size() > 256 ?  256 : strfData->size();
                    LOGW("strf size of %zu: dumping first %d", strfData->size(), s);
                }
                for(int i = 0; i < s; ++i) {
                    LOGV("%02d: %02x", i, strfData->data()[i]);
                }
...
*/
        ...
        if (size < 3) {
            LOGE("error codec data size for vorbis: %zd", size);
            return ERROR_UNSUPPORTED;
        }
        ...
        if (size <= 3 + len1 + len2) {
            LOGE("error codec data size for vorbis: %zd %zd %zd", size, len1, len2);
            return ERROR_UNSUPPORTED;
        }
