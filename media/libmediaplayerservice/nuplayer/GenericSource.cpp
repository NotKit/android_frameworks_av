...
void NuPlayer::GenericSource::readBuffer(
    ...
    for (size_t numBuffers = 0; numBuffers < maxBuffers; ) {
        ...
        if (couldReadMultiple) {
            ...
            if (track->mSource == NULL) {
                ALOGW("%s track->mSource == NULL, return", trackType == MEDIA_TRACK_TYPE_VIDEO ? "video" : "audio");
                return;
            }
#endif
...
void NuPlayer::GenericSource::changeMaxBuffersInNeed(size_t *maxBuffers) {
    ...
    if (isTS()) {
        *maxBuffers = 8;  // add for ALPS03121128, 4K video not smooth
         return;
    }
...
void NuPlayer::GenericSource::setParams(const sp<MetaData>& meta)
{
    ...
    int32_t decoderError = 0;
    if (isTS() && meta->findInt32(kKeyDecoderError, &decoderError)) {
        Mutex::Autolock _l(mSourceLock);
        if (decoderError == 1 && mAudioTrack.mSource != NULL) {
            mAudioTrack.mSource->start(meta.get());
            // add for ALPS02974226 to avoid block when seek, because no buffers
            // be parsed after this err caused no return when seek audio.
            mAudioTrack.mSource = NULL;
            ALOGE("audio decoder err!");
        } else if (decoderError == 2 && mVideoTrack.mSource != NULL) {
            mVideoTrack.mSource->start(meta.get());
            mVideoTrack.mSource = NULL;
